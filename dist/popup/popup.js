export class PopupManager {
    ...
    initialize() {
        ...
        if (this.chrome.runtime) {
            this.chrome.runtime.onMessage.addListener(this.handleStatusUpdate.bind(this));
        }
    }
    /**
    ...
    async startDownload() {
        ...
        try {
            ...
            // Show error in UI
            if (this.statusElement) {
                this.statusElement.textContent = `Error: ${error instanceof Error ? error.message : "Unknown error"}`;
            }
            // Re-enable button
            if (this.downloadButton) {
    ...
    getActiveTabs() {
        return new Promise((resolve) => {
            ...
            this.chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => resolve(tabs));
        });
    }
    /**
