export class BackgroundService {
    ...
    async startDownloadProcess(tabId) {
        ...
        if (this.chrome.tabs) {
            this.chrome.tabs.sendMessage(tabId, { action: "scrapeConversation" }, (response) => {
                if (this.chrome.runtime?.lastError) {
                    console.error("Error sending message to content script:", this.chrome.runtime.lastError);
                    this.handleError(tabId, "Failed to communicate with content script");
                }
            });
        }
